...
@block
def connector(din, we, inbusy, rd, rdout, outbusy, dout, hfull):
    
    @always_comb
    def connectorLogic():
        din.next = dout
        we.next = rdout
        rd.next = not outbusy and not inbusy and hfull
        
    return instances()

#Module with encapsulated bus
@block
...
def fifo_convert(DATA=8, ADDR=4):
    ...
    fifo_inst.convert(hdl='Verilog', header=hdlcfg.header, directory=hdlcfg.hdl_path,
                     name='fifo_' + str(DATA)+'_'+ str(ADDR)+'_'+ str(UPPER)+'_'+ str(LOWER)+'_'+ str(OFFSET))
